...
class EmotionClass:
    ...
    def Emotion_Sensing(self, input):
        ...
        if len(input) > 100:
            input = input[:100]
            print(f"输入过长，将仅使用前100个字符。原始长度: {len(original_input)}")
        ...
        EmotionChain = ChatPromptTemplate.from_messages([
            ("system", prompt_emotion),
            ("user", input)
        ]) | llm
        ...
        try:
            ...
            if EmotionChain is not None:
                ...
                raise ValueError("EmotionChain 未正确实例化。")
            self.Emotion = result
            return result
